...
export const AnalyticsReportPage: React.FC = () => {
  ...
  // Học sinh tiêu biểu (Tính toán dựa trên điểm thi và số bài đã nộp thực tế)
  const topStudents = useMemo(() => {
    ...
    // Tính toán số liệu thực tế cho từng học sinh dựa trên danh sách bài nộp (submissions)
    const scoredStudents = list.map((st) => {
      // Tìm tất cả bài nộp của học sinh này
      const studentSubs = submissions.filter((sub: any) => {
        const matchCode = Boolean(sub.student_code && st.student_code && sub.student_code === st.student_code);
        const matchName = Boolean(
          sub.student_name &&
          st.full_name &&
          sub.student_name.trim().toLowerCase() === st.full_name.trim().toLowerCase() &&
          (!sub.class_name || !st.class_name || sub.class_name === st.class_name)
        );
        return matchCode || matchName;
      });

      const submissionCount = studentSubs.length;
      let totalNormalizedScore = 0;
      let maxScore = 0;
      let excellentCount = 0; // Số bài >= 8.0

      if (submissionCount > 0) {
        studentSubs.forEach((sub: any) => {
          const max = Number(sub.max_score) || 10;
          const score = Number(sub.score) || 0;
          const norm = Number(((score / max) * 10).toFixed(1));
          totalNormalizedScore += norm;
          if (norm > maxScore) maxScore = norm;
          if (norm >= 8.0) excellentCount++;
        });
      }

      const avgScore =
        submissionCount > 0 ? Number((totalNormalizedScore / submissionCount).toFixed(1)) : 0;

      // Điểm XP thi đua (dựa trên hồ sơ hoặc tính từ bài làm)
      const calculatedXp = st.xp || (submissionCount * 50 + Math.round(avgScore * 10));
    ...
    // Sắp xếp:
    // 1. Học sinh đã làm bài kiểm tra trước (submissionCount > 0)
    // 2. Điểm trung bình cao nhất (avgScore giảm dần)
    // 3. Số bài nộp nhiều hơn (submissionCount giảm dần)
    // 4. Điểm thưởng XP cao nhất (calculatedXp giảm dần)
    // 5. Điểm số tối đa (maxScore giảm dần)
    scoredStudents.sort((a, b) => {
      ...
      if (b.calculatedXp !== a.calculatedXp) {
        return b.calculatedXp - a.calculatedXp;
      }
      return b.maxScore - a.maxScore;
    });

    return scoredStudents.slice(0, 8);
  }, [students, submissions, selectedGrade, selectedClass]);
      ...
      <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Vinh danh học sinh dẫn đầu theo Điểm trung bình bài kiểm tra thực tế & Điểm thưởng XP tích lũy
            </p>
          </div>
          <div className="text-xs text-slate-600 font-bold bg-slate-50 px-3.5 py-1.5 rounded-xl border border-slate-200 self-start sm:self-auto">
            {selectedClass !== 'all'
              ? `Xếp hạng: ${selectedClass}`
              : selectedGrade !== 'all'
              ? `Xếp hạng: Khối ${selectedGrade}`
              : 'Xếp hạng: Toàn trường THCS'}
          </div>
        </div>

        {topStudents.length === 0 ? (
          <div className="text-center py-8 text-slate-400 text-xs">
            Chưa có dữ liệu học sinh trong danh sách đã chọn.
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
            {topStudents.map((st: any, idx: number) => {
              ...
              return (
                <div
                  key={st.id || `${st.student_code}_${idx}`}
                  className={`relative p-4 rounded-2xl border transition-all duration-200 hover:shadow-md flex flex-col justify-between gap-3 ${
                    rank === 1
                      ? 'bg-gradient-to-br from-amber-50/80 via-yellow-50/30 to-white border-amber-300 ring-1 ring-amber-200'
                      : rank === 2
                      ? 'bg-gradient-to-br from-slate-50/90 via-slate-50/40 to-white border-slate-300'
                      : rank === 3
                      ? 'bg-gradient-to-br from-amber-100/30 via-orange-50/20 to-white border-amber-200'
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="shrink-0">
                      {rank === 1 ? (
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-400 to-amber-500 text-white flex items-center justify-center font-black shadow-md shadow-amber-200 ring-2 ring-amber-300">
                          <Crown className="w-5 h-5 text-amber-950 fill-amber-950" />
                        </div>
                      ) : rank === 2 ? (
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-slate-300 to-slate-400 text-white flex items-center justify-center font-black shadow-xs ring-2 ring-slate-200 text-sm">
                          <Medal className="w-5 h-5 text-slate-800" />
                        </div>
                      ) : rank === 3 ? (
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-700 to-amber-600 text-white flex items-center justify-center font-black shadow-xs ring-2 ring-amber-400/50 text-sm">
                          <Medal className="w-5 h-5 text-amber-100" />
                        </div>
                      ) : (
                        <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-700 flex items-center justify-center font-black text-sm border border-slate-200">
                          #{rank}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="font-bold text-slate-900 text-xs sm:text-sm truncate" title={st.full_name}>
                        {st.full_name}
                      </div>
                      <div className="text-[11px] text-slate-500 flex items-center gap-1.5 mt-0.5">
                        <span className="font-medium">{st.class_name || 'Lớp 7A1'}</span>
                        <span>•</span>
                        <span className="font-mono text-ocean-700 font-semibold">{st.student_code}</span>
                      </div>
                    </div>
                  </div>

                  {/* Thông tin Điểm số & Thành tích thực tế */}
                  <div className="pt-2.5 border-t border-slate-100/80 flex items-center justify-between gap-2 text-xs">
                    {hasSub ? (
                      <>
                        <div className="flex items-center gap-1.5">
                          <span className="text-[11px] text-slate-500 font-medium">ĐTB:</span>
                          <span
                        ...
                        <div className="text-[11px] text-slate-500 font-medium flex items-center gap-1">
                          ...
                          <span>{st.submissionCount} bài</span>
                        </div>
                        <div className="text-[11px] font-bold text-amber-600 flex items-center gap-0.5">
                          <Zap className="w-3 h-3 text-amber-500 fill-amber-500" />
                          <span>{st.calculatedXp} XP</span>
                        </div>
                      </>
                    ) : (
                      <div className="w-full flex items-center justify-between text-[11px] text-slate-400">
                        <span>Chưa nộp bài nào</span>
