...
export const ExamGradingPage: React.FC = () => {
  ...
  const [isAdvancedFilterOpen, setIsAdvancedFilterOpen] = useState<boolean>(false); // Ẩn/hiện bộ lọc nâng cao
...
👉 Các em học sinh vui lòng đăng nhập vào trang web để hoàn thành và nộp bài sớm nhé!`;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 pb-16">
      ...
      {/* 4. BỘ LỌC TÌM KIẾM TINH GỌN (CORE FILTERS) & BỘ LỌC MỞ RỘNG (ADVANCED FILTERS) */}
      <div className="bg-white p-3 rounded-2xl border border-slate-200 shadow-xs space-y-2.5">
        {/* Hàng lọc chính: Đề thi, Khối, Lớp, Nút mở rộng, Ô tìm kiếm */}
        <div className="flex flex-col sm:flex-row items-center gap-2.5 flex-wrap">
          ...
          <select
            ...
            className="px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-ocean-500 max-w-xs w-full sm:w-auto cursor-pointer"
          >
          ...
          <select
            ...
            className="px-3 py-2 rounded-xl border border-slate-200 text-xs font-bold text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-ocean-500 w-full sm:w-auto cursor-pointer"
          >
          ...
          {/* Nút Bật / Tắt Bộ Lọc Mở Rộng */}
          <button
            type="button"
            onClick={() => setIsAdvancedFilterOpen((prev) => !prev)}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold border transition cursor-pointer shrink-0 ${
              isAdvancedFilterOpen || (lateFilter !== 'all' || scoreFilter !== 'all' || testTypeFilter !== 'all')
                ? 'bg-ocean-50 border-ocean-300 text-ocean-800'
                : 'bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100'
            }`}
            title="Bấm để mở/ẩn các bộ lọc nâng cao theo hạn nộp, mức điểm, bài thi thử"
          >
            <Filter className="w-3.5 h-3.5 text-ocean-600" />
            <span>Bộ Lọc Nâng Cao</span>
            {(lateFilter !== 'all' || scoreFilter !== 'all' || testTypeFilter !== 'all') && (
              <span className="w-4 h-4 rounded-full bg-ocean-600 text-white text-[10px] font-black flex items-center justify-center">
                {(lateFilter !== 'all' ? 1 : 0) + (scoreFilter !== 'all' ? 1 : 0) + (testTypeFilter !== 'all' ? 1 : 0)}
              </span>
            )}
          ...
          <div className="relative flex-1 min-w-[200px] w-full">
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-ocean-500 bg-slate-50"
            />
            <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
          </div>
        </div>

        {/* Khung Bộ Lọc Mở Rộng (Chỉ hiển thị khi bấm mở hoặc đang có bộ lọc nâng cao hoạt động) */}
        {isAdvancedFilterOpen && (
          <div className="pt-2.5 border-t border-slate-100 flex items-center gap-2.5 flex-wrap animate-in fade-in slide-in-from-top-1 duration-200 bg-slate-50/50 p-2.5 rounded-xl border border-slate-200/60">
            <span className="text-[11px] font-black text-slate-500 uppercase tracking-wider flex items-center gap-1">
              <span>⚡ Lọc chuyên sâu:</span>
            </span>
            ...
            <select
              ...
              className="px-3 py-1.5 rounded-xl border border-slate-200 text-xs font-bold text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-ocean-500 cursor-pointer"
            >
            ...
            {(lateFilter !== 'all' || scoreFilter !== 'all' || testTypeFilter !== 'all') && (
              <button
                ...
                className="flex items-center gap-1 text-[11px] font-bold text-rose-600 hover:text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 px-2.5 py-1.5 rounded-xl transition cursor-pointer"
              >
